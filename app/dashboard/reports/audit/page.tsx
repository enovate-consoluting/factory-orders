...
const ACTION_TYPES = [
  ...
  { value: 'inventory_create', label: 'Inventory Created' },
  { value: 'inventory_update', label: 'Inventory Updated' },
  { value: 'inventory_delete', label: 'Inventory Deleted' },
  { value: 'inventory_received', label: 'Inventory Received' },
  { value: 'inventory_archived', label: 'Inventory Archived' },
  { value: 'photo_upload_complete', label: 'Photo Uploaded' },
];
...
export default function AuditLogPage() {
  ...
  useEffect(() => {
    ...
    if (parsed.role !== 'super_admin' && parsed.role !== 'system_admin') {
  ...
  const getActionIcon = (actionType: string) => {
    if (actionType.includes('deleted')) return <Trash2 className="w-4 h-4 text-red-500" />;
    if (actionType.includes('created') || actionType.includes('added')) return <Package className="w-4 h-4 text-green-500" />;
    if (actionType.includes('updated') || actionType.includes('price')) return <Edit className="w-4 h-4 text-blue-500" />;
    if (actionType.includes('routed')) return <Send className="w-4 h-4 text-purple-500" />;
    if (actionType.includes('status') || actionType.includes('received') || actionType.includes('archived')) return <CheckCircle className="w-4 h-4 text-amber-500" />;
    if (actionType.includes('invoice') || actionType.includes('payment')) return <FileText className="w-4 h-4 text-indigo-500" />;
    if (actionType.includes('photo') || actionType.includes('upload')) return <Package className="w-4 h-4 text-cyan-500" />;
  ...
  const getActionColor = (actionType: string) => {
    ...
    if (actionType.includes('received') || actionType.includes('archived')) return 'bg-amber-50 border-amber-200';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <h3 className="font-medium text-gray-900 mb-1">No audit entries found</h3>
            <p className="text-sm text-gray-500">
              {hasActiveFilters ? 'Try adjusting your filters' : 'Activity will appear here as changes are made'}
            </p>
          </div>
        ) : (
          <>
            <div className="space-y-2">
              {filteredEntries.map((entry) => {
                ...
                return (
                  <div
                    key={entry.id}
                    className={`rounded-xl border overflow-hidden ${getActionColor(entry.action_type)}`}
                  >
                    <div
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-1 text-xs">
                            ...
                            {entry.target_id && (
                              <span className="text-gray-500 font-mono">{entry.target_id.slice(0, 8)}...</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="px-4 pb-4 border-t border-gray-200/50 bg-white/30">
                        <div className="pt-3">
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm mb-3">
                            <div>
                              <span className="text-gray-500 block text-xs">Target ID</span>
                            ...
                            <div>
                              ...
                              <span className="text-gray-700 text-xs">{formatDate(timestamp)}</span>
                            </div>
                          </div>
