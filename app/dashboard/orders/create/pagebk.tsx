...
interface OrderProduct {
  ...
  description?: string  // Order-specific description
  uploadedFiles?: Array<{
    name: string
    type: string
    size: number
  ...
  const addSelectedProducts = async () => {
    ...
    for (const productId of selectedIds) {
      ...
      setOrderProducts(prev => [...prev, { 
        ...
        items,
        description: ''  // Initialize empty description for order-specific details
      ...
      for (const orderProduct of orderProducts) {
        const { data: createdProduct, error: productError } = await supabase
          ...
          .insert({
            ...
            description: orderProduct.description || null  // Save order-specific description
          })
          .select()
          .single()
                    ...
                    <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                      <div className="flex items-center gap-2 flex-1">
                        <input
                          ...
                          className="flex-1 sm:flex-none sm:w-32 px-3 py-2 border border-gray-300 rounded-lg text-gray-900 placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                          min="0"
                        />
                      ...
                      <button
                        ...
                        className="w-full sm:w-auto px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium text-sm transition-all duration-200 shadow-sm hover:shadow-md"
                      >
                        Apply to All
                      </button>
                    </div>
                  ...
                  <div key={productIndex} className="border border-gray-200 rounded-lg p-4">
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        ...
                        {/* Order-specific description field */}
                        <div className="mt-2">
                          <label className="block text-sm font-medium text-gray-600 mb-1">
                            Product Description (Order Specific)
                          </label>
                          <input
                            type="text"
                            ...
                            onChange={(e) => {
                              const updated = [...orderProducts]
                              updated[productIndex].description = e.target.value
                              setOrderProducts(updated)
                            }}
                            placeholder="e.g., Black diamond hoodie, Silver hoodie with logo"
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Add specific details for this {orderProduct.product.title.toLowerCase()} in this order
                          </p>
                        </div>
                      </div>
                      <button
